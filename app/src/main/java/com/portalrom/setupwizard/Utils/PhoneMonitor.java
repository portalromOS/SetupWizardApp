...
import static com.portalrom.setupwizard.SetupWizard.LOGV;
...
public class PhoneMonitor {
    ...
    private void updatePhoneStateTrackers() {
        int i = 0;
        int[] subIds = mSubscriptionManager.getSubscriptionIds(0);
    ...
    public ServiceState getServiceStateForSubscriber(int subId) {
        return mTelephony.getServiceState();
    }
    ...
    public boolean simMissing() {
        ...
        List<SubscriptionInfo> subs = mSubscriptionManager.getAccessibleSubscriptionInfoList();
        if (subs != null) {
    ...
    public boolean singleSimInserted() {
        return mSubscriptionManager.getAccessibleSubscriptionInfoList().size() == 1;
    }

    // We only care that each slot has a sim
    public boolean allSimsInserted() {
        int simSlotCount = mTelephony.getSimState();
        ...
        return simSlotCount == mSubscriptionManager.getAccessibleSubscriptionInfoList().size();
    }

    @SuppressLint("MissingPermission")
    public boolean isMultiSimDevice() {
        return mTelephony.isMultiSimSupported() == TelephonyManager.MULTISIM_ALLOWED;
    }

    public boolean isGSM(int subId) {
        return mTelephony.createForSubscriptionId(subId).getPhoneType() == PHONE_TYPE_GSM;
    }
    ...
    private String getPhoneTypeName() {
        return getPhoneTypeName(mTelephony != null ? mTelephony.getPhoneType() : -1);
    }
    ...
    private String getNetworkTypeName() {
        return mTelephony != null ? mTelephony.getNetworkOperatorName() : "";
    }

    private String getSubscriptionSimStateName(int subId) {
        @SuppressLint("MissingPermission") SubscriptionInfo subInfo = mSubscriptionManager.getActiveSubscriptionInfo(subId);
        if (subInfo == null) {
    ...
    private String getPhoneState() {
        ...
        for (SubscriptionInfo subId : mSubscriptionManager.getAccessibleSubscriptionInfoList()) {
            states.append(" ").append(getPhoneState(subId.getCardId()));
        }
